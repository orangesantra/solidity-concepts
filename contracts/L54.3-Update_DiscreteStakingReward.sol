...
contract DiscreteStakingRewards {
    ...
    mapping(address => uint) rewardIndexofUser;
    ...
    function updateRewardIndex(uint reward, address dueTo) external {
        rewardToken.transferFrom(msg.sender, address(this), reward);
        rewardIndex += (reward * MULTIPLIER) / totalSupply;
        rewardIndexofUser[dueTo]+= (reward*MULTIPLIER)/ totalSupply;
    }

    function _calculateRewards(address account) private view returns (uint) {
        uint shares = balanceOf[account];
        return (shares * (rewardIndexofUser[account] - rewardIndexOf[account])) / MULTIPLIER;
    }
    ...
    function _updateRewards(address account) private {
        ...
        rewardIndexOf[account] = rewardIndexofUser[account];
    }
