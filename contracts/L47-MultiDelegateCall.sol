...
contract MultiDelegatecall {
    ...
    function multiDelegatecall(
        ...
        for (uint i; i < data.length; i++) {
            ...
            // when the above line is executed inside testMultiDelegatecll contract
            // due to inhertance, the address(this) will be of TestMultiDelegtaecall contract
            // also it's true because functions passed in data[i] are only available for TestMultiDelegateCall contract
            // not inside MultiDelegatecall.
            if (!ok) {
